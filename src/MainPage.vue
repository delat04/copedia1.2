...
function addNewMarker(lat, lng) {
  ...
  const customIcon = L.icon({
    iconUrl: 'public/images/home.png', // URL of the image
  ...
  if (allowedExtensions.includes(fileExtension)) {
    ...
    axios.post('http://localhost:3001/save-image', formData)
        .then(response => {
          ...
          // Update newMarkerData with the saved image URL
...
window.deleteMarker = async function (longitude, latitude) {
  try {
    ...
    // Optionally, update your markers array or UI after successful deletion
  } catch (error) {
...
function createPopupContent(marker, index) {
  ...
  return `
    ...
    <div class="d-flex justify-content-between">
      ...
      <router-link :to="\`/houseDetails/${marker.properties.id}/${encodeURIComponent(marker.properties.title)}/${marker.properties.price}/${encodeURIComponent(marker.properties.description)}/${encodeURIComponent(JSON.stringify(marker.properties.image))}\`" class="btn btn-info">Details</router-link>
    </div>
  `;
}
...
onMounted(async () => {
  adjustMapHeight();
  try {
    axios.get("./Netlify/get-data.js") // Updated URL for Netlify function
        .then(response => {
